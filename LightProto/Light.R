...
get_latitudes_var <- function(files, latitudes = c(-65, -55), varname) {
  ...
  furrr::future_map_dfr(files, f, .id = "month")  
}
...
d <- bind_rows(l) %>% inner_join(areas) %>% mutate(valuebyarea = median * area_m2)
...
d <- readRDS("PAR.rds")
d$par <- d$valuebyarea
ggplot(d) + geom_bar(aes(x = month, y = par, fill = factor(decade)), position = "dodge", stat = "identity") + 
  geom_errorbar(aes(x = month, ymin= q10 * area_m2, ymax= q90 * area_m2, group = decade), width = 0.2, position = position_dodge(0.5)) +
...
d <- readRDS("KD490.rds")
d$kd490 <- d$valuebyarea
ggplot(d) + geom_bar(aes(x = month, y = kd490, fill = factor(decade)), position = "dodge", stat = "identity") + 
  geom_errorbar(aes(x = month, ymin= q10 * area_m2, ymax= q90 * area_m2, group = decade), width = 0.2, position = position_dodge(0.5)) +
  facet_grid(sector~lat)  + 
